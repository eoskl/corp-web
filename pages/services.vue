  ...
  <div class="mx-auto max-w-2xl lg:max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
    <nav class="flex justify-between items-center">
      ...
      <div class="flex items-start relative">
        <div class="snipcart-total-price mr-4 text-3xl"></div> -
        <div class="snipcart-items-count mx-4 text-3xl"></div>
        <ShoppingBagIcon class="h-8 w-8 snipcart-checkout cursor-pointer" />
      </div>
    </nav>
  </div>
  <div id="product" class="mx-auto max-w-2xl py-4 px-4 sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-2 lg:gap-x-8 lg:px-8">
    <div class="lg:max-w-lg">
      <LazyClientOnly>
        <RadioGroup v-model="selected" class="flex">
          ...
          >
            <div
              :class="[checked ? 'bg-amber-600 text-white ' : 'bg-white']"
              class="capitalize text-md relative flex cursor-pointer rounded-lg px-5 py-4 shadow-md focus:outline-none hover:bg-amber-700 hover:text-white"
              @click="setLanguage(lang, lang === 'fr' ? { actions: { continue_shopping: 'Revenir a votre magnifique magasin override runtime' } } : {})"
            >
      ...
      <div class="mt-4">
        <h1 class="text-3xl font-bold tracking-tight text-gray-900 sm:text-4xl">{{ product.data.name }}</h1>
      </div>
      <section class="mt-4">
        <div class="flex items-center">
          <p class="text-lg text-gray-900 sm:text-xl">{{ product.data.price }}</p>
          <div class="ml-4 border-l border-gray-300 pl-4">
            <div class="flex items-center">
              <div>
                ...
                <p class="sr-only">{{ product.data.reviews.average }} out of 5 stars</p>
              </div>
              <p class="ml-2 text-sm text-gray-500">{{ product.data.reviews.totalCount }} reviews</p>
            </div>
          </div>
        </div>
        <div class="mt-4 space-y-6">
          <p class="text-base text-gray-500">{{ product.data.description }}</p>
        </div>
        <div class="lg:col-start-1 lg:max-w-lg lg:self-start">
          <div class="mt-10">
            <button
              ...
              type="button"
              class="snipcart-add-item flex w-full items-center justify-center rounded-md border border-transparent bg-amber-600 py-3 px-8 text-base font-medium text-white hover:bg-amber-700 focus:outline-none"
            >
              Add to bag
            </button>
          </div>
    ...
    <div class="mt-10 lg:col-start-2 lg:row-span-2 lg:mt-0 lg:self-center">
      <div class="aspect-w-1 aspect-h-1 overflow-hidden rounded-lg">
        <img :src="product.data.src" :alt="product.data.alt" class="h-full w-full object-cover object-center">
      </div>
    </div>
  </div>
</template>
...
const handleCancelStore = () => {
  if (offStore.value) {
    offStore.value();
    console.log('Store subscription canceled');
  }
};
